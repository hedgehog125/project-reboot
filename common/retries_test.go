...
func TestCalculateBackoff(t *testing.T) {
	...
	for range 1000 { // I love testing randomness :grimacing:
		shortBackoff1 := CalculateBackoff(0, 100*time.Millisecond, 2)
		shortBackoff2 := CalculateBackoff(1, 100*time.Millisecond, 2)
		longBackoff1 := CalculateBackoff(0, 30*time.Second, 2)
		longBackoff2 := CalculateBackoff(1, 30*time.Second, 2)
