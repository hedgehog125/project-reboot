...
// Mutate calls f(ctx, m).
func (f TwoFactorActionFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TwoFactorActionMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TwoFactorActionMutation", m)
}
...
// The UserMessengerFunc type is an adapter to allow the use of ordinary
// function as UserMessenger mutator.
type UserMessengerFunc func(context.Context, *ent.UserMessengerMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f UserMessengerFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.UserMessengerMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.UserMessengerMutation", m)
}
